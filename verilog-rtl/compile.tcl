...
write_file -format verilog -no_implicit -hierarchy -output $MAPPED_PATH/${TOP_MODULE}.v;
...
lmap CLK_N $CLK_NAME CLK_P $CLK_PERIOD {
    set  CLK_SKEW          [expr  $CLK_P* 0.05]
    set  CLK_TRAN          [expr  $CLK_P * 0.01]
    set  CLK_SRC_LATENCY   [expr  $CLK_P * 0.1]
    set  CLK_LATENCY       [expr  $CLK_P * 0.1]

    create_clock -period $CLK_P [get_ports  $CLK_N]

    set_ideal_network      [get_ports  $CLK_N];  #这两句是废话，不过强化clk是ideal_network
    set_dont_touch_network [get_ports  $CLK_N];  #告诉DC不对clk做优化
    set_drive    0         [get_ports  $CLK_N];  #设置驱动，0代表无穷大
    set_clock_uncertainty  -setup   $CLK_SKEW        [get_ports  $CLK_N];  #偏移
    set_clock_transition   -max     $CLK_TRAN        [get_ports  $CLK_N];  #斜率反转，非直角翻转
    set_clock_latency -source -max  $CLK_SRC_LATENCY [get_ports  $CLK_N];  #PCB版上晶振到芯片引脚的延迟
    set_clock_latency -max          $CLK_LATENCY     [get_ports  $CLK_N];  #芯片引脚到内部触发器的延迟
}
error_info

if {[llength $CLK_NAME] > 1} {
    set DESIGN_CLOCKS [all_clocks]
    foreach_in_collection CLK_N $DESIGN_CLOCKS {
        set_false_path -from $CLK_N -to [remove_from_collection [all_clocks] $CLK_N]
    }
}
...
#set   INPUT_DELAY       [expr  $CLK_PERIOD * (1 - $TIME_BUDGET)]
#set_input_delay     $INPUT_DELAY     -clock  $CLK_NAME   $ALL_IN_EXCEPT_CLK; #相对CLK_NAME对哪些端口设置延迟
lmap CLK_N $CLK_NAME CLK_P $CLK_PERIOD {
    set INPUT_DELAY [expr  $CLK_P * (1 - $TIME_BUDGET)]
    set_input_delay $INPUT_DELAY -clock $CLK_N $ALL_IN_EXCEPT_CLK; #相对CLK_NAME对哪些端口设置延迟
    #set_input_delay    -min   0    -clock $CLK_N   $ALL_IN_EXCEPT_CLK
}
...
#=====================================================
#set   OUTPUT_DELAY     [expr  $CLK_PERIOD * (1 - $TIME_BUDGET)]
...
lmap CLK_N $CLK_NAME CLK_P $CLK_PERIOD {
    set OUTPUT_DELAY [expr $CLK_P * (1 - $TIME_BUDGET)]
    set_output_delay $OUTPUT_DELAY -clock $CLK_N [all_outputs]
}
...
#=====================================================
lmap CLK_N $CLK_NAME CLK_P $CLK_PERIOD {
    group_path -name $CLK_N -weight 5 -critical_range [expr $CLK_P * 0.1]
    #指定一个group名字，指定一个权重（路径差，需要尽最大优化）指定一个范围，对这个范围内的路径进行优化，一般不超过周期%10。通常分为下面几个组：输入，输出，之间的路径
    group_path -name "INPUTS_$CLK_N"  -from [all_inputs]  -critical_range [expr $CLK_P * 0.1]
    group_path -name "OUTPUTS_$CLK_N" -to   [all_outputs] -critical_range [expr $CLK_P * 0.1]
    group_path -name "COMB_$CLK_N"    -from [all_inputs]  -to [all_outputs] -critical_range [expr $CLK_P * 0.1]	
}
...
#=====================================================
set_host_options -max_cores 16; #8 might be max
set_optimize_registers true; #流水线寄存器优化
